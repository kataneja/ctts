...
public class AdminController extends FwAbstractController {
	...
	public ResponseEntity<GetStaffSecurityProfileResponse> getStaffProfile(@ModelAttribute("wams") String wams, HttpServletRequest httpRequest , HttpSession httpSession){
		...
		try{
			...
			responseCargo.setPrfl_id("user");
			responseCargo.setShowAdminPanel(false);
			responseCargo.setShowApproveTimesheet(false);
			responseCargo.setShowDashboard(true);
			responseCargo.setShowReports(false);
			responseCargo.setShowSubmitTimeoff(true);
			responseCargo.setShowSubmitTimesheet(true);
			responseCargo.setShowUserManagement(false);
